...
export default function MapLibreGISMap({ deviceLocations, geoJsonData }) {
  ...
  // Funtcion to determine the  marker colour based on the water content
  ...
  useEffect(() => {
    ...
    const center = [deviceLocations[0].lng, deviceLocations[0].lat]; 
    ...
    mapRef.current.on("load", () => {
      ...
      // Adds a circle layer for water content visualization
      ...
      // Adds a symbol layer for markers
      mapRef.current.addLayer({
        id: "device-markers",
        type: "circle",
        source: "devices",
